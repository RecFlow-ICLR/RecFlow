...
from models import DSSM_AuxRanking
from dataset import Prerank_Train_Auxiliary_Ranking_Dataset
...
if __name__ == '__main__':
  ...
  model = DSSM_AuxRanking(args.emb_dim, args.seq_len, device, id_cnt_dict).to(device)
  ...
  n_flow_photos = n_flows * 10
  
  #training
  for epoch in range(args.epochs):
    for n_day in range(num_of_train_csv):
      
      train_dataset = Prerank_Train_Auxiliary_Ranking_Dataset(
      ...
      for iter_step, inputs in enumerate(train_loader):
        ...
        logits, flow_logits = model.forward_train(inputs_LongTensor) #b*1,b*p
        ...
        logits_repeat = logits.repeat([1,n_flow_photos]) #b*p
        
        bpr_logits = logits_repeat - flow_logits
