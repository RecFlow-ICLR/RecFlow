...
from dataset import Prerank_Train_Data_Dist_Shift_All_Dataset
...
if __name__ == '__main__':
  ...
  remap_daily_prefix = os.path.join(prefix, "remap_daily")
  path_to_train_csv_lst = []
  with open("./file.txt", mode='r') as f:
    lines = f.readlines()
    for line in lines:
      tmp_csv_path = os.path.join(remap_daily_prefix, line.strip()+'.feather')
  ...
  for epoch in range(args.epochs):
    for n_day in range(num_of_train_csv):
      train_dataset = Prerank_Train_Data_Dist_Shift_All_Dataset(
      ...
      for iter_step, inputs in enumerate(train_loader):
        ...
        label = torch.FloatTensor(inputs[-1].numpy()).to(device) #b*k
