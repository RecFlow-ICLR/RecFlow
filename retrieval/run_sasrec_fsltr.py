...
from dataset import Recall_Train_SASRec_FSLTR_Dataset
...
if __name__ == '__main__':
  ...
  for epoch in range(args.epochs):
    for n_day in range(num_of_train_csv):
      ...
      train_dataset = Recall_Train_SASRec_FSLTR_Dataset(
      ...
      for iter_step, inputs in enumerate(train_loader):
        ...
        logits = model.forward_fsltr(inputs_LongTensor) #b*k
        
        priority = torch.FloatTensor(inputs[-1].numpy()).to(device) #b*k
        ...
        loss = F.binary_cross_entropy_with_logits(logits_diff, torch.ones_like(logits_diff), weight=weight, reduction='sum') / weight.sum()
  ...
  path_to_save_model=f"./checkpoints/bs-{args.batch_size}_lr-{args.lr}_{args.neg_num}_{args.flow_negs}_{args.flow_neg_nums}_{args.tag}.pkl"
