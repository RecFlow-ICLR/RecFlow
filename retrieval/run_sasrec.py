...
from dataset import Recall_Train_SASRec_Dataset
...
def parse_args():
  ...
  parser.add_argument('--neg_num', type=int, default=50, help='number of negative samples')
...
if __name__ == '__main__':
  ...
  for epoch in range(args.epochs):
    for n_day in range(num_of_train_csv):
      ...
      train_dataset = Recall_Train_SASRec_Dataset(
      ...
      for iter_step, inputs in enumerate(train_loader):
        ...
        pos_logits = tgt_logits.repeat_interleave(args.neg_num)
        
        loss = -loss_fn(pos_logits-neg_logits).mean()
  ...
  path_to_save_model=f"./checkpoints/bs-{args.batch_size}_lr-{args.lr}_{args.neg_num}_{args.tag}.pkl"
